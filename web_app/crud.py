...
def add_keys_helper(model2, model3, result):
    query2 = dict(model2.query.with_entities(
        model2.job_title_code,
        model2.job_title_name).distinct().all())
    for x in result:
        ...
        if 'job_title' in x.keys():
            dep_code = model2.query.filter(
                model2.job_title_name == x['job_title']
                ).first().department_code
            dep_name = model3.query.filter(
                model3.department_code == dep_code).first().department_name
...
def from_sql(row):
    ...
    if 'country_code' in new_new_data.keys():
        URL = (f'https://restcountries.eu/rest/v2/alpha?codes={row.country_code};{row.country_code};{row.country_code}')
...
def list_all_employees(model1, model2, model3):
    ...
    result = add_keys_helper(
        model2,
        model3,
        list(map(from_sql, query)))
    return result


def list_all_join(model1, model2):
    query = db.session.query(model1).filter(
        model2.department_code == model1.department_code).all()
    result = list(map(from_sql, query))
    query2 = dict(model2.query.with_entities(
        model2.department_name,
        model2.department_code).distinct().all())
    for x in result:
        if 'department_code' in x.keys():
            x['department_name'] = list(
                query2.keys())[list(
                    query2.values()).index(x['department_code'])]
...
def list_with_var1(model, keyz, date_limit, var1):
    query = model.query.filter(and_(keyz == var1,
    date_limit > datetime.date.today())).all()
    result = list(map(from_sql, query))
    return result


def check_with_var1(model, model2, model3, date_limit, var1):
    query = model.query.filter(or_(date_limit == var1,
    date_limit < datetime.date.today())).all()
    result = add_keys_helper(model2, model3, list(
        map(from_sql, query)))
    return result
...
def find_by_join(model1, model2, model3, var2):
    query = db.session.query(model1).filter(
        model3.department_name == var2).filter(
            model3.department_code == model2.department_code).filter(
                model2.job_title_code == model1.job_title_code).all()
    result = add_keys_helper(
        model2, 
        model3, 
        list(map(from_sql, query)))
...
def find_by_join_2(model1, model2, var1):
    query = db.session.query(
        model1).filter(
            model2.department_name == var1).filter(
                model2.department_code == model1.department_code).all()
    result = list(map(from_sql, query))
    query2 = dict(
        model2.query.with_entities(
            model2.department_name,
            model2.department_code).distinct().all())
    for x in result:
        if 'department_code' in x.keys():
